...
int main(void)
{
  ...
  int insideReading = 100;
  int outsideReading = 0;
  int lightReading = 400;
...
uint16_t readAnalog(int pin)
{
  ...
  uint8_t adsc_mask = 0 | _BV(ADSC);

 while (ADCSRA & adsc_mask != 0){
  ...
  // wait for ADSC bit to go high
  while (ADCSRA & adsc_mask == 0){
    // wait for ADSC bit to set
  }
...
void stopISR(){
  //based on finISR, but only for timer1
...
void setWindowAngle(int angle)
{
  ...
  else if (angle >= WINDOW_CLOSED && angle <= WINDOW_OPEN)
  {
   // unlockWindow();
    ...
    //convert angle to ticks by multiplying by clockCyclesPerMicrosecond
